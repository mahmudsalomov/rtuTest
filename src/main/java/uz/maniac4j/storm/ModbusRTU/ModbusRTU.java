...
//        RtuNettyModbusClient rtuClient=new RtuNettyModbusClient(config,serialPortProvider);
        ...
        while(!success) {
            try {
                modbusClient.Connect("192.168.176.93", 502);
//                boolean[] response = modbusClient.ReadCoils(2, 20);
                int[] responseint = modbusClient.ReadHoldingRegisters(1, 2);
...
//                for(int i = 0; i < responseint.length; ++i) {

                    System.out.println(uz.maniac4j.storm.modbus.client.ModbusClient.ConvertRegistersToFloat(responseint, ModbusClient.RegisterOrder.HighLow));
